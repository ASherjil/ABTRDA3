...
bool DPDKEal::ealInit(const std::string& bdf, int lcore) noexcept {
    ...
    if (g_vdevs.empty()) {
        addAllowedBdf(bdf);   // BDF mode only; AF_XDP vdevs register in prepare()
    }
    ...
    std::vector<std::string> args = {
        ...
        lcoreStr,  "-n",
        "4",       "--file-prefix",
        prefix,    "--force-max-simd-bitwidth=" + std::to_string(kMaxSimdBitwidth)};
    if (!g_vdevs.empty()) {
...
bool DPDKEal::bindToVfio(const std::string& bdf) noexcept {
    ...
    if (pci::currentDriver(bdf) == "vfio-pci") {
        return true;
    }
