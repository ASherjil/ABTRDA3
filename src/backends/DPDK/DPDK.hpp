...
template <DpdkMode M, std::uint16_t QueueId = 0, std::uint16_t NbRxDesc = 256, std::uint16_t NbTxDesc = 256,
          std::uint16_t BurstSize = 32, std::uint32_t NumMbufs = 8191,
...
public:
    ...
    [[nodiscard]] bool init(bool doWaitLink = true, bool bifurcated = false, bool afxdpPMD = false,
                            bool applyDeferral = false) noexcept;
...
DPDK<M, QueueId, NbRxDesc, NbTxDesc, BurstSize, NumMbufs, MaxFrame>::DPDK(std::string_view ifname, int lcore,
...
DPDK<M, QueueId, NbRxDesc, NbTxDesc, BurstSize, NumMbufs, MaxFrame>::operator=(DPDK&& o) noexcept {
    if (this != &o) {
        shutdown();
        m_ifname          = std::move(o.m_ifname);
        m_driver          = std::move(o.m_driver);
...
template <DpdkMode M, std::uint16_t QueueId, std::uint16_t NbRxDesc, std::uint16_t NbTxDesc,
          std::uint16_t BurstSize, std::uint32_t NumMbufs, std::uint16_t MaxFrame>
bool DPDK<M, QueueId, NbRxDesc, NbTxDesc, BurstSize, NumMbufs, MaxFrame>::prepare(bool bifurcated,
                                                                                  bool afxdpPMD) noexcept {
    if (m_prepared) {
        return true;
    }
    ...
    if (m_afxdpPmd) {
        ...
        addVdev(vname + ",iface=" + m_ifname + ",start_queue=0,queue_count=1,mode=drv,busy_budget=64");
    ...
    if (m_pci.empty()) {
        m_pci = pci::bdfFromName(m_ifname);
    }
    if (m_pci.empty()) {
        std::fprintf(stderr,
                     "[DPDK] %s: cannot resolve PCI BDF (no netdev, and name is "
...
template <DpdkMode M, std::uint16_t QueueId, std::uint16_t NbRxDesc, std::uint16_t NbTxDesc,
          std::uint16_t BurstSize, std::uint32_t NumMbufs, std::uint16_t MaxFrame>
bool DPDK<M, QueueId, NbRxDesc, NbTxDesc, BurstSize, NumMbufs, MaxFrame>::init(bool doWaitLink,
                                                                               bool bifurcated, bool afxdpPMD,
                                                                               bool applyDeferral) noexcept {
    m_applyDeferral = applyDeferral;
    if (!prepare(bifurcated, afxdpPMD)) {
        return false;
    }

    if (!ealInit(m_pci, m_lcore)) {
        return false;
    }
    ...
    if (sid < 0) {
        sid = static_cast<int>(rte_socket_id());
    }

    char poolName[32];
    std::snprintf(poolName, sizeof(poolName), "mbufpool_%u", m_port);
    m_pool = rte_pktmbuf_pool_create(poolName, NumMbufs, MBUF_CACHE, 0, RTE_MBUF_DEFAULT_BUF_SIZE, sid);
    if (!m_pool) {
    ...
    if (m_linkSpeeds != 0) {
        ...
        std::fprintf(stderr,
                     "[DPDK] %s: autoneg advertisement restricted "
    ...
    std::uint16_t nb_rxd = m_nbRxDesc;
    std::uint16_t nb_txd = NbTxDesc;
    ...
    if (m_afxdpPmd) {
        if (m_applyDeferral) {
            napi::applyDeferralSysfs(m_ifname.c_str(), napi::kDeferHardIrqs, napi::kGroFlushTimeoutNs,
                                     "DPDK");
        } else {
...
bool DPDK<M, QueueId, NbRxDesc, NbTxDesc, BurstSize, NumMbufs, MaxFrame>::waitLink() noexcept {
    ...
    for (int i = 0; i < 200; ++i) {
        ...
        if (rte_eth_link_get_nowait(m_port, &link) == 0 && link.link_status == RTE_ETH_LINK_UP) {
            break;
        }
        if (i == 20 || (i > 20 && (i % 30) == 0)) {
            const int e = rte_eth_dev_set_link_up(m_port);
            if (e != 0 && e != -ENOTSUP) {
    ...
    std::fprintf(stderr, "[DPDK] %s port %u %s %u Mbps  MAC %02x:%02x:%02x:%02x:%02x:%02x\n",
                 m_ifname.c_str(), m_port, link.link_status == RTE_ETH_LINK_UP ? "UP" : "DOWN",
                 link.link_speed, m_mac[0], m_mac[1], m_mac[2], m_mac[3], m_mac[4], m_mac[5]);
    if (link.link_status != RTE_ETH_LINK_UP) {
        std::fprintf(stderr,
                     "[DPDK] %s: WARN link still DOWN after ~20s — "
    ...
    // RX-ring overrun; rx_nombuf = mempool exhausted. On igc imissed always reads 0.)
    {
        ...
        if (rte_eth_stats_get(m_port, &st) == 0) {
            std::fprintf(
                ...
                m_ifname.c_str(), static_cast<unsigned long long>(st.opackets),
                static_cast<unsigned long long>(st.ipackets), static_cast<unsigned long long>(st.obytes),
                static_cast<unsigned long long>(st.ibytes), static_cast<unsigned long long>(st.imissed),
                static_cast<unsigned long long>(st.ierrors), static_cast<unsigned long long>(st.oerrors),
...
template <DpdkMode M, std::uint16_t QueueId, std::uint16_t NbRxDesc, std::uint16_t NbTxDesc,
          std::uint16_t BurstSize, std::uint32_t NumMbufs, std::uint16_t MaxFrame>
std::array<std::uint8_t, 6> DPDK<M, QueueId, NbRxDesc, NbTxDesc, BurstSize, NumMbufs, MaxFrame>::macAddress()
    const noexcept {
    return m_mac;
}
...
template <DpdkMode M, std::uint16_t QueueId, std::uint16_t NbRxDesc, std::uint16_t NbTxDesc,
          std::uint16_t BurstSize, std::uint32_t NumMbufs, std::uint16_t MaxFrame>
void DPDK<M, QueueId, NbRxDesc, NbTxDesc, BurstSize, NumMbufs, MaxFrame>::prefillRing(
...
template <DpdkMode M, std::uint16_t QueueId, std::uint16_t NbRxDesc, std::uint16_t NbTxDesc,
          std::uint16_t BurstSize, std::uint32_t NumMbufs, std::uint16_t MaxFrame>
inline void DPDK<M, QueueId, NbRxDesc, NbTxDesc, BurstSize, NumMbufs, MaxFrame>::release() noexcept
    requires (M == DpdkMode::RxOnly || M == DpdkMode::RxTx)
{
    rte_pktmbuf_free(m_pendingRx);
    m_pendingRx = nullptr;
...
template <DpdkMode M, std::uint16_t QueueId, std::uint16_t NbRxDesc, std::uint16_t NbTxDesc,
          std::uint16_t BurstSize, std::uint32_t NumMbufs, std::uint16_t MaxFrame>
inline std::uint8_t* DPDK<M, QueueId, NbRxDesc, NbTxDesc, BurstSize, NumMbufs, MaxFrame>::acquire(
    ...
    requires (M == DpdkMode::TxOnly || M == DpdkMode::RxTx)
{
    if (m_txReuseInline && frameLen <= INLINE_SAFE_LEN) {
        if (!m_txReuse) [[unlikely]] {
            ...
            if (m_txTemplateLen) {
                std::memcpy(rte_pktmbuf_mtod(m_txReuse, std::uint8_t*), m_txTemplate.data(), m_txTemplateLen);
            }
        }
...
template <DpdkMode M, std::uint16_t QueueId, std::uint16_t NbRxDesc, std::uint16_t NbTxDesc,
          std::uint16_t BurstSize, std::uint32_t NumMbufs, std::uint16_t MaxFrame>
inline void DPDK<M, QueueId, NbRxDesc, NbTxDesc, BurstSize, NumMbufs, MaxFrame>::commit() noexcept
    requires (M == DpdkMode::TxOnly || M == DpdkMode::RxTx)
{
...
template <DpdkMode M, std::uint16_t QueueId, std::uint16_t NbRxDesc, std::uint16_t NbTxDesc,
          std::uint16_t BurstSize, std::uint32_t NumMbufs, std::uint16_t MaxFrame>
inline bool DPDK<M, QueueId, NbRxDesc, NbTxDesc, BurstSize, NumMbufs, MaxFrame>::send(
...
template <DpdkMode M, std::uint16_t QueueId, std::uint16_t NbRxDesc, std::uint16_t NbTxDesc,
          std::uint16_t BurstSize, std::uint32_t NumMbufs, std::uint16_t MaxFrame>
void DPDK<M, QueueId, NbRxDesc, NbTxDesc, BurstSize, NumMbufs, MaxFrame>::setLinkSpeeds(
    std::uint32_t speedsMask) noexcept {
...
template <DpdkMode M, std::uint16_t QueueId, std::uint16_t NbRxDesc, std::uint16_t NbTxDesc,
          std::uint16_t BurstSize, std::uint32_t NumMbufs, std::uint16_t MaxFrame>
void DPDK<M, QueueId, NbRxDesc, NbTxDesc, BurstSize, NumMbufs, MaxFrame>::setSymmetricQueues(
    bool on) noexcept {
...
template <DpdkMode M, std::uint16_t QueueId, std::uint16_t NbRxDesc, std::uint16_t NbTxDesc,
          std::uint16_t BurstSize, std::uint32_t NumMbufs, std::uint16_t MaxFrame>
void DPDK<M, QueueId, NbRxDesc, NbTxDesc, BurstSize, NumMbufs, MaxFrame>::setNbRxDesc(
...
template <DpdkMode M, std::uint16_t QueueId, std::uint16_t NbRxDesc, std::uint16_t NbTxDesc,
          std::uint16_t BurstSize, std::uint32_t NumMbufs, std::uint16_t MaxFrame>
void DPDK<M, QueueId, NbRxDesc, NbTxDesc, BurstSize, NumMbufs, MaxFrame>::setDevargs(
    std::string_view devargs) noexcept {
    m_devargs = devargs;
}
