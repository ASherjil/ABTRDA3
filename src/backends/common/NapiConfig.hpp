...
inline std::size_t putAttrStr(char* buf, std::size_t off, std::uint16_t type, const char* s) {
...
inline std::uint16_t resolveFamily(int fd, const char* name) {
    ...
    for (auto* rh = reinterpret_cast<nlmsghdr*>(rbuf); NLMSG_OK(rh, n); rh = NLMSG_NEXT(rh, n)) {
        if (rh->nlmsg_type == NLMSG_ERROR || rh->nlmsg_type == NLMSG_DONE) {
            return 0;
        }
        // Walk attrs after the genlmsghdr looking for CTRL_ATTR_FAMILY_ID (u16).
        auto* attr = reinterpret_cast<nlattr*>(reinterpret_cast<char*>(NLMSG_DATA(rh)) + GENL_HDRLEN);
        int   rem  = static_cast<int>(rh->nlmsg_len) - static_cast<int>(NLMSG_HDRLEN) -
                  static_cast<int>(GENL_HDRLEN);
        for (; rem >= static_cast<int>(NLA_HDRLEN) && rem >= attr->nla_len;
             rem -= NLA_ALIGN(attr->nla_len),
             attr = reinterpret_cast<nlattr*>(reinterpret_cast<char*>(attr) + NLA_ALIGN(attr->nla_len))) {
            if (attr->nla_type == CTRL_ATTR_FAMILY_ID) {
                return *reinterpret_cast<std::uint16_t*>(reinterpret_cast<char*>(attr) + NLA_HDRLEN);
            }
        }
    }
    return 0;
...
inline bool setBusyPoll(std::uint32_t napiId, std::uint32_t deferHardIrqs, std::uint64_t groFlushNs,
                        std::uint64_t irqSuspendNs, const char* tag = "AFXDP") {
...
inline NapiParams getBusyPoll(std::uint32_t napiId, const char* tag = "AFXDP") {
    ...
    for (auto* rh = reinterpret_cast<nlmsghdr*>(rbuf); NLMSG_OK(rh, n); rh = NLMSG_NEXT(rh, n)) {
        ...
        for (; rem >= static_cast<int>(NLA_HDRLEN) && rem >= attr->nla_len;
            ...
            const std::size_t plen     = static_cast<std::size_t>(attr->nla_len) - NLA_HDRLEN;
            const auto        readUint = [&]() -> std::uint64_t {
                if (plen >= 8) {
                    std::uint64_t v = 0;
                    std::memcpy(&v, payload, 8);
                    return v;
                }
                if (plen >= 4) {
                    std::uint32_t v = 0;
...
inline bool writeNetdevKnob(const char* ifname, const char* knob, std::uint64_t value, const char* tag) {
    char path[128];
    ...
    char       val[24];
    const int  n  = std::snprintf(val, sizeof(val), "%llu", static_cast<unsigned long long>(value));
    const bool ok = (::write(fd, val, static_cast<std::size_t>(n)) == n);
    if (!ok) {
        fmt::print(stderr, "[{}] WARN: write {}={}: {}\n", tag, path, value, std::strerror(errno));
    }
    ::close(fd);
    return ok;
}
...
inline bool applyDeferralSysfs(const char* ifname, std::uint32_t deferHardIrqs, std::uint64_t groFlushNs,
                               const char* tag) {
...
inline void detachXdpProgram(const char* ifname, const char* tag) {
    ...
    for (const std::uint32_t mode : {static_cast<std::uint32_t>(XDP_FLAGS_DRV_MODE),
                                     static_cast<std::uint32_t>(XDP_FLAGS_SKB_MODE), 0u}) {
        struct {
            nlmsghdr  nlh;
            ifinfomsg ifi;
            char      attrs[64];
    ...
    ::close(fd);
    fmt::print(stderr,
               "[{}] {}: cleared any pre-existing XDP program "
...
inline CombinedResult ensureCombinedOne(const char* ifname, const char* tag) {
    ...
    if (!ethtoolIoctl(&ch) || ch.max_combined == 0) {
        ::close(fd);
        fmt::print(stderr,
                   "[{}] WARN: {} has no combined channels (driver lacks "
    ...
    if (ch.combined_count == 1) {
        ::close(fd);
        fmt::print(stderr,
                   "[{}] OK: {} already at 1 RX queue (combined=1) — "
    ...
    if (!ok) {
        fmt::print(stderr,
                   "[{}] FAIL: {} combined={} (want 1) — RX may not reach "
                   "the XSK; run: sudo ethtool -L {} combined 1\n",
                   tag, ifname, rb.combined_count, ifname);
        return CombinedResult::Failed;
    }
    fmt::print(stderr,
               "[{}] {} steered to 1 RX queue (combined=1) — link "
...
inline void printXskProcessValidation(const char* ifname, bool deferralExpected, const char* tag) {
    ...
    while (dirent* de = ::readdir(dir)) {
        char*      end = nullptr;   // NOLINT(misc-const-correctness) strtol takes char**
        ...
        fmt::print(stderr,
                   "[{}] {}: XSK fd={}: ZERO-COPY={} prefer_busy_poll={} "
                   "busy_poll={}us (XDP_OPTIONS=0x{:x}; budget is set-only) {}\n",
                   tag, ifname, fd, m.zeroCopy ? "YES" : "NO(copy-mode!)", m.preferBusyPoll, m.busyPollUs,
                   m.xdpOptionsFlags,
                   good ? "[OK: real AF_XDP ZC + busy-poll]" : "<-- NOT the intended ZC+busy-poll mode");
    }
    ::closedir(dir);
    if (!found) {
        fmt::print(stderr,
                   "[{}] {}: WARN: no (new) AF_XDP socket found in this "
