...
private:
  ...
  [[gnu::always_inline, gnu::hot]]
  inline void completeTx() noexcept requires (M != AFXDPMode::RxOnly);

  [[gnu::always_inline, gnu::hot]]
  inline void kickTx() noexcept requires (M != AFXDPMode::RxOnly);
...
inline RxFrame AFXDP<M, NumRxFrames, NumTxFrames, FrameSize, NeedWakeup>::tryReceive() noexcept requires (M != AFXDPMode::TxOnly) {
  ...
  __u32 idx;
  unsigned int rcvd = xsk_ring_cons__peek(&m_xsk.rx, 1, &idx);
  if (!rcvd) {
  ...
  return { .data   = { static_cast<std::uint8_t*>(xsk_umem__get_data(m_umem.buffer, m_pendingRxAddr)), desc->len },
           .sec    = 0,
           .nsec   = 0,
           .status = 1 };
}
...
template<AFXDPMode M, std::uint32_t NumRxFrames, std::uint32_t NumTxFrames, std::uint32_t FrameSize, bool NeedWakeup>
inline void AFXDP<M, NumRxFrames, NumTxFrames, FrameSize, NeedWakeup>::completeTx() noexcept requires (M != AFXDPMode::RxOnly) {
  ...
  __u32 idx;
  if (unsigned int rcvd = xsk_ring_cons__peek(&m_umem.cq, BATCH_SIZE, &idx)) {
...
void AFXDP<M, NumRxFrames, NumTxFrames, FrameSize, NeedWakeup>::xskConfigureUmem() {
  ...
  if (int ret = xsk_umem__create(&m_umem.umem, m_umem.buffer, umemSize, &m_umem.fq, &m_umem.cq, &umem_cfg)) {
...
void AFXDP<M, NumRxFrames, NumTxFrames, FrameSize, NeedWakeup>::xskPopulateFillRing() {
  ...
  for (std::uint32_t i = 0; i < got; i++) {
    *xsk_ring_prod__fill_addr(&m_umem.fq, idx++) = static_cast<std::uint64_t>(TX_POOL_FRAMES + i) * FrameSize;
  }
...
void AFXDP<M, NumRxFrames, NumTxFrames, FrameSize, NeedWakeup>::xskConfigureSocket() {
  ...
  fmt::print(stderr, "[AFXDP] {} | queue={} frames={} (tx={} rx={})\n",
             m_bindMode, m_queueId, NUM_FRAMES, TX_POOL_FRAMES, RX_POOL_FRAMES);
}
