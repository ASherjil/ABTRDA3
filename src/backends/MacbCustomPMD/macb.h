...
/* ABTRDA3: force 64-bit DMA descriptor extension (ARM64 Zynq UltraScale+) */
...
/* ABTRDA3: Register access macros stripped — the kernel versions dereference
 * `struct macb *port` and call macb_reg_readl/macb_reg_writel function pointers
 * that we do not provide. In our PMD, register access goes through
 * AXIBackend::readReg(offset) / AXIBackend::writeReg(offset, value) directly.
 *
 * Original kernel macros (for reference, removed):
 *   macb_readl, macb_writel        → AXIBackend::readReg(MACB_##reg)
 *   gem_readl,  gem_writel         → AXIBackend::readReg(GEM_##reg)
 *   queue_readl, queue_writel      → AXIBackend::readReg(GEM_##reg(queue_idx))
 *   gem_readl_n, gem_writel_n      → AXIBackend::readReg(GEM_##reg + idx * 4)
 *   macb_or_gem_readl/writel       → use gem_ variants directly (always GEM here)
 */
...
/* ABTRDA3: Kernel driver state structs removed (macb_tx_skb, macb_stats,
 * gem_stats, queue_stats, macb_queue, macb, phylink/clk/ptp hooks, etc.).
 * They depended on sk_buff, napi_struct, spinlock_t, phylink_pcs, clk,
 * ptp_clock, work_struct — none of which exist in our userspace PMD.
 * Our equivalents live in Cadence_GEM.hpp (the PMD class) using AXIBackend
 * for register access and HugepageBuffer for DMA-capable memory.
 */
