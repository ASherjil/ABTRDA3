...
inline TestConfig loadConfig(const char* path) {
    ...
    if (nicTunerStr == "off") {
        cfg.nicTunerMode = NicTunerMode::Off;
    } else if (nicTunerStr == "nfs_safe") {
        cfg.nicTunerMode = NicTunerMode::NfsSafe;
    } else {
        cfg.nicTunerMode = NicTunerMode::Full;
    }
    ...
    cfg.recQueueCapacity = static_cast<std::size_t>(
        tbl["single_recorder"]["queue_capacity"].value_or(1u << 20));
