...
template <TxRing Tx>
inline void run_txgen(Tx& tx, const TestConfig& cfg, std::uint32_t count, const std::stop_token& stop) {
    ...
    const std::uint64_t t_start = txgen_now_ns();
    std::uint32_t       sent    = 0;
    ...
    const std::uint64_t t_end = txgen_now_ns();
    ...
    const double pps       = (elapsed_s > 0) ? static_cast<double>(sent) / elapsed_s : 0;
