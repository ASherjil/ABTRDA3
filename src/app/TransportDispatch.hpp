...
inline void dispatchMode(Tx& tx, Rx& rx, RunMode mode, const TestConfig& cfg, std::uint32_t count,
...
[[nodiscard]] inline int withTransport(std::string_view name, Fn&& fn) {
    if (name == PacketMmapTraits::kName) {
        return fn(std::type_identity<PacketMmapTraits>{});
    }
    if (name == AfxdpTraits::kName) {
        return fn(std::type_identity<AfxdpTraits>{});
    }
#ifdef ABTRDA3_HAVE_DPDK
    if (name == DpdkTraits::kName) {
        return fn(std::type_identity<DpdkTraits>{});
    }
#endif
#ifdef ABTRDA3_HAVE_VERBS
    if (name == VerbsTraits::kName) {
        return fn(std::type_identity<VerbsTraits>{});
    }
#endif
#ifdef ABTRDA3_HAVE_EFVI
    if (name == EtherFabricTraits::kName) {
        return fn(std::type_identity<EtherFabricTraits>{});
    }
#endif
#ifdef ABTRDA3_HAVE_I210
    if (name == I210Traits::kName) {
        return fn(std::type_identity<I210Traits>{});
    }
#endif
    // Compiled-in names already returned above, so reaching one of these means
    // this binary was built without it.
    for (const std::string_view gated : {"dpdk", "verbs", "ef_vi", "intel_i210"}) {
        if (name == gated) {
            fmt::println(stderr,
                         "Error: transport '{}' is not compiled into this binary — "
                         "reconfigure with -DABTRDA3_WITH_ALL=ON (or the matching "
                         "-DABTRDA3_WITH_DPDK/VERBS/EFVI/I210=ON) and rebuild",
                         name);
            return 1;
        }
    }
...
template <class Tr>
[[nodiscard]] inline int runModes(const TestConfig& cfg, const RoleConfig& role, RunMode mode,
                                  std::uint32_t count, std::stop_token stop) {
    if (!Tr::preflight(cfg, role)) {
        return 1;
    }
    const char* roleName = runModeName(mode);

    auto bringUp = [&](auto& nic) -> bool {
        if (Tr::init(nic, cfg, role)) {
            return true;
        }
...
[[nodiscard]]
inline int runTransport(const TestConfig& cfg, const RoleConfig& role, RunMode mode, std::uint32_t count,
                        std::stop_token stop) {
...
template <class Tr>
[[nodiscard]] inline int runSingleFor(const TestConfig& cfg, const std::stop_token& stop) {
    if constexpr (!Tr::kSingleCapable) {
        fmt::println(stderr,
                     "Error: single_recorder: transport '{}' is not supported "
                     "(needs two ports in one process)",
                     Tr::kName);
        return 1;
    } else {
        if (!Tr::preflight(cfg, cfg.client) || !Tr::preflight(cfg, cfg.server)) {
            return 1;
        }
...
inline int runSingleRecorder(const TestConfig& cfg, std::stop_token stop) {
    ...
    if (cfg.server.transport != transport) {
        fmt::println(stderr,
                     "Error: single_recorder needs client.transport == "
                     "server.transport (Tx=client port, Rx=server port); "
                     "got tx='{}' rx='{}'",
                     transport, cfg.server.transport);
        return 1;
    }
    if (cfg.recHotPathCore == cfg.recRxCore || cfg.recHotPathCore == cfg.recRecorderCore ||
        cfg.recRxCore == cfg.recRecorderCore) {
        fmt::println(stderr,
                     "Error: single_recorder needs three DISTINCT cores "
                     "(tx={} rx={} recorder={})",
    ...
    fmt::println("[SingleRecorder] {} | Tx={} (core {}) Rx={} (core {}) Hist core {} "
                 "duration={}s",
                 transport, cfg.client.interface, cfg.recHotPathCore, cfg.server.interface, cfg.recRxCore,
                 cfg.recRecorderCore, cfg.recDurationSec);
