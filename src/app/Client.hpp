...
inline void run_client(Tx& tx, Rx& rx, const TestConfig& cfg, std::stop_token stop) {
    ...
    const std::uint64_t durationCyc = static_cast<std::uint64_t>(tscHz *
                                                                 static_cast<double>(cfg.clientDurationSec));
            ...
            if (const RxFrame rxf = rx.tryReceive(); !rxf.data.empty()) [[unlikely]] {
                ...
                if (rxf.data.size() >= kFrameMinBytes &&
                    ...
                    if (rxSeq == seqNet) [[likely]] {   // our echo
                        ...
                        if (warmup < kWarmupDiscard) {
                            ...
                            if constexpr (prof::kDebugProfiling) {
                                if (sample > outlierThreshCyc && outlierLogged < 64) {
                                    fmt::print(stderr, "[outlier] #{:<11} t={:8.1f}ms  rtt={:7.2f}us\n",
                                               recorded, static_cast<double>(loopTsc - start) * 1e3 / tscHz,
                        ...
                        if constexpr (prof::kDebugProfiling) {
                            rxStats.record(prof::cycles() - rxStart);
                        }
                        received = true;
                        break;
                    }
